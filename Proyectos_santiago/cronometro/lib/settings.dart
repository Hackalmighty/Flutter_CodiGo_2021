...
class _SettingsState extends State<Settings> {
  ...
  void cambiarTTrabajo(int i) {
    setState(() {
      tTrabajo += i;
    });
  }

  void cambiarTBreak(int i) {
    setState(() {
      tBreak += i;
    });
  }

  void cambiarTDescanzo(int i) {
    setState(() {
      tDescanzo += i;
    });
  }

  cargarValoresPorDefecto() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      tTrabajo = prefs.getInt('trabajo') ?? 30;
      tBreak = prefs.getInt('break') ?? 5;
      tDescanzo = prefs.getInt('descanzo') ?? 15;
    });
  }
