...
class DBManager {
  ...
  Future<Database> get database async {
    ...
    /// Crear base de datos y asignarla
    return _database;
  }
  ...
  initDB() async {
    ...
    return await openDatabase(
      ...
      onCreate: (Database db, int version) async {
        await db.execute(
            'CREATE TABLE Dog (id INTEGER PRIMARY KEY, nameDog TEXT, colorDog TEXT, imageDog TEXT)');
      },
    );
  }
  ...
  Future<List<Dog>> getAllDogs() async {
    ...
    List<Dog> listDogs = res.isNotEmpty ? res.map((juanito) =>
        Dog.fromJson(juanito)).toList() : [];
    print(listDogs);
    return listDogs;
  }
  ...
  insertDogRaw(Dog perrito) async {
    ...
    final res = await db.rawInsert(
        "INSERT INTO Dog(id, nameDog, colorDog, imageDog) VALUES(${perrito
            .id},'${perrito.nameDog}','${perrito.colorDog}','${perrito
            .imageDog}')"
    );
    print(res);
    return res;
  ...
  Future<int> updateDog(Dog perrito) async {
    ...
    final res = await db.update(
        "Dog", perrito.toJson(), where: 'id = ?', whereArgs: [perrito.id]);
    return res;
  }
